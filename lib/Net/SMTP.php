...
class Net_SMTP
{
    ...
    protected function send($data)
    {
        ...
        if (!$result || PEAR_is_Error($result)) {
    ...
    {
        if (PEAR_is_Error($error = $this->put($command))) {
            return $error;
        }
        if (PEAR_is_Error($error = $this->parseResponse($valid))) {
            return $error;
        }
    ...
    public function connect($timeout = null, $persistent = false)
    {
         ...
         */
        if ($this->timeout > 0) {
            if (PEAR_is_Error($error = $this->setTimeout($this->timeout))) {
                return $error;
            }
        }

        if (PEAR_is_Error($error = $this->parseResponse(220))) {
            return $error;
        }
        ...
        if (PEAR_is_Error($error = $this->negotiate())) {
            return $error;
        }
    ...
    {
        if (PEAR_is_Error($error = $this->put('QUIT'))) {
            return $error;
        }
        if (PEAR_is_Error($error = $this->parseResponse(221))) {
            return $error;
        }
        if (PEAR_is_Error($error = $this->socket->disconnect())) {
            return PEAR::raiseError(
    ...
    {
        if (PEAR_is_Error($error = $this->put('EHLO', $this->localhost))) {
            return $error;
        }

        if (PEAR_is_Error($this->parseResponse(250))) {
            /* If the EHLO failed, try the simpler HELO command. */
            if (PEAR_is_Error($error = $this->put('HELO', $this->localhost))) {
                return $error;
            }
            if (PEAR_is_Error($this->parseResponse(250))) {
    ...
    public function auth($uid, $pwd , $method = '', $tls = true, $authz = '')
    {
        ...
        if ($tls && version_compare(PHP_VERSION, '5.1.0', '>=')
            ...
            if (PEAR_is_Error($result = $this->put('STARTTLS'))) {
                return $result;
            }
            if (PEAR_is_Error($result = $this->parseResponse(220))) {
                return $result;
            }
            ...
            if (PEAR_is_Error($result = $this->socket->enableCrypto(true, $crypto_method))) {
                return $result;
            } elseif ($result !== true) {
        ...
        if (empty($method)) {
            if (PEAR_is_Error($method = $this->getBestAuthMethod())) {
    ...
    {
        if (PEAR_is_Error($error = $this->put('AUTH', 'DIGEST-MD5'))) {
            return $error;
        }
        /* 334: Continue authentication request */
        if (PEAR_is_Error($error = $this->parseResponse(334))) {
        ...
        if (PEAR_is_Error($error = $this->put($auth_str))) {
            return $error;
        }
        /* 334: Continue authentication request */
        if (PEAR_is_Error($error = $this->parseResponse(334))) {
         ...
         * it. */
        if (PEAR_is_Error($error = $this->put(''))) {
            return $error;
        }
        /* 235: Authentication successful */
        if (PEAR_is_Error($error = $this->parseResponse(235))) {
            return $error;
        }
    }
    ...
    {
        if (PEAR_is_Error($error = $this->put('AUTH', 'CRAM-MD5'))) {
            return $error;
        }
        /* 334: Continue authentication request */
        if (PEAR_is_Error($error = $this->parseResponse(334))) {
    ...
    {
        if (PEAR_is_Error($error = $this->put('AUTH', 'LOGIN'))) {
            return $error;
        }
        /* 334: Continue authentication request */
        if (PEAR_is_Error($error = $this->parseResponse(334))) {
        ...
        if (PEAR_is_Error($error = $this->put(base64_encode($uid)))) {
            return $error;
        }
        /* 334: Continue authentication request */
        if (PEAR_is_Error($error = $this->parseResponse(334))) {
        ...
        if (PEAR_is_Error($error = $this->put(base64_encode($pwd)))) {
            return $error;
        }
    ...
    {
        if (PEAR_is_Error($error = $this->put('AUTH', 'PLAIN'))) {
            return $error;
        }
        /* 334: Continue authentication request */
        if (PEAR_is_Error($error = $this->parseResponse(334))) {
    ...
    {
        if (PEAR_is_Error($error = $this->put('HELO', $domain))) {
            return $error;
        }
        if (PEAR_is_Error($error = $this->parseResponse(250))) {
            return $error;
        }
    ...
    public function mailFrom($sender, $params = null)
    {
        ...
        if (PEAR_is_Error($error = $this->put('MAIL', $args))) {
            return $error;
        }
        if (PEAR_is_Error($error = $this->parseResponse(250, $this->pipelining))) {
            return $error;
        }
    ...
    public function rcptTo($recipient, $params = null)
    {
        ...
        if (PEAR_is_Error($error = $this->put('RCPT', $args))) {
            return $error;
        }
        if (PEAR_is_Error($error = $this->parseResponse(array(250, 251), $this->pipelining))) {
            return $error;
        }
    ...
    public function data($data, $headers = null)
    {
        ...
        if (PEAR_is_Error($error = $this->put('DATA'))) {
            return $error;
        }
        if (PEAR_is_Error($error = $this->parseResponse(354))) {
            return $error;
        }
        ...
        if (!is_null($headers)) {
            ...
            if (PEAR_is_Error($result = $this->send($headers . "\r\n\r\n"))) {
                return $result;
            }
        }
        ...
        if (is_resource($data)) {
            ...
            while (strlen($line = fread($data, 8192)) > 0) {
                ...
                $this->quotedata($line);
                if (PEAR_is_Error($result = $this->send($line))) {
                    return $result;
                }
            }
            ...
            for ($offset = 0; $offset < $size;) {
                ...
                if (PEAR_is_Error($result = $this->send($chunk))) {
                    return $result;
                }
        ...
        if (PEAR_is_Error($result = $this->send($terminator))) {
            return $result;
        }
    ...
    {
        if (PEAR_is_Error($error = $this->put('SEND', "FROM:<$path>"))) {
            return $error;
        }
        if (PEAR_is_Error($error = $this->parseResponse(250, $this->pipelining))) {
            return $error;
    ...
    {
        if (PEAR_is_Error($error = $this->put('SOML', "FROM:<$path>"))) {
            return $error;
        }
        if (PEAR_is_Error($error = $this->parseResponse(250, $this->pipelining))) {
            return $error;
    ...
    {
        if (PEAR_is_Error($error = $this->put('SAML', "FROM:<$path>"))) {
            return $error;
        }
        if (PEAR_is_Error($error = $this->parseResponse(250, $this->pipelining))) {
            return $error;
    ...
    {
        if (PEAR_is_Error($error = $this->put('RSET'))) {
            return $error;
        }
        if (PEAR_is_Error($error = $this->parseResponse(250, $this->pipelining))) {
            return $error;
    ...
    public function vrfy($string)
    {
        ...
        if (PEAR_is_Error($error = $this->put('VRFY', $string))) {
            return $error;
        }
        if (PEAR_is_Error($error = $this->parseResponse(array(250, 252)))) {
            return $error;
        }
    ...
    {
        if (PEAR_is_Error($error = $this->put('NOOP'))) {
            return $error;
        }
        if (PEAR_is_Error($error = $this->parseResponse(250))) {
            return $error;
